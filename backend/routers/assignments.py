...
async def create_assignment(
    ...
    return AssignmentResponse(
        id=new_assignment.id, # type: ignore
        kelas_id=new_assignment.kelas_id, # type: ignore
        title=new_assignment.title, # type: ignore
        description=new_assignment.description, # type: ignore
        questions=[QuestionRead.model_validate(q) for q in new_assignment.questions],
        deadline=new_assignment.deadline, # type: ignore
        max_score=new_assignment.max_score, # type: ignore
        minimal_score=new_assignment.minimal_score, # type: ignore
        is_published=new_assignment.is_published, # type: ignore
        created_at=new_assignment.created_at # type: ignore
    )
...
async def get_class_assignments(
    ...
    if not is_teacher: # type: ignore
        assignments = [a for a in assignments if a.is_published] # type: ignore

    return [
        AssignmentResponse(
            id=assignment.id, # type: ignore
            kelas_id=assignment.kelas_id, # type: ignore
            title=assignment.title, # type: ignore
            description=assignment.description, # type: ignore
            questions=[QuestionRead.model_validate(q) for q in assignment.questions], # type: ignore
            deadline=assignment.deadline, # type: ignore
            max_score=assignment.max_score, # type: ignore
            minimal_score=assignment.minimal_score, # type: ignore
            is_published=assignment.is_published, # type: ignore
...
async def get_assignment_detail(
    ...
    if not is_teacher:
        ...
        if not assignment.is_published: # type: ignore
    ...
    return AssignmentDetailResponse(
        ...
        is_published=assignment.is_published, # type: ignore
        created_at=assignment.created_at, # type: ignore
        submission_count=len(assignment.submissions), # type: ignore
...
async def update_assignment(
    ...
    if request.deadline is not None:
        assignment.deadline = request.deadline # type: ignore
    if request.max_score is not None:
        assignment.max_score = request.max_score # type: ignore
    if request.minimal_score is not None:
        assignment.minimal_score = request.minimal_score # type: ignore
            ...
            if question_update.id is not None:
                ...
                if existing_question:
                    if question_update.question_text is not None:
                        existing_question.question_text = question_update.question_text # type: ignore
                    if question_update.reference_answer is not None:
                        existing_question.reference_answer = question_update.reference_answer # type: ignore
                    if question_update.points is not None:
                        existing_question.points = question_update.points # type: ignore
                    existing_question.question_order = idx + 1 # type: ignore
            else:
...
async def submit_answer_typing(
    ...
    if assignment.deadline and datetime.utcnow() > assignment.deadline: # type: ignore
