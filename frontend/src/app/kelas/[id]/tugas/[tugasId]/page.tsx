...
function AssignmentDetailContent() {
	...
	useEffect(() => {
		const submittingKey = `submitting_${assignmentId}`;
		const isSubmittingStored = sessionStorage.getItem(submittingKey);
		if (isSubmittingStored === 'true') {
			setIsSubmitting(true);
			toast.loading("Proses pengumpulan masih berlangsung...", {
				id: 'submitting-progress',
				duration: Infinity,
			});
		}
	}, [assignmentId]);
	useEffect(() => {
		const submittingKey = `submitting_${assignmentId}`;
		if (!isSubmitting) {
			const storedState = sessionStorage.getItem(submittingKey);
			if (storedState === 'true') {
				const timer = setTimeout(() => {
					sessionStorage.removeItem(submittingKey);
					toast.dismiss('submitting-progress');
				}, 1000);
				
				return () => clearTimeout(timer);
			}
		}
	}, [isSubmitting, assignmentId]);
	...
	const handleSubmit = async () => {
		...
		// Save submitting state to session storage
		sessionStorage.setItem(submittingKey, 'true');
		toast.loading("Mengumpulkan jawaban...", {
			id: 'submitting-progress',
			duration: Infinity,
		});
	...
	return (
		<div className="min-h-screen flex flex-col bg-gray-50">
			...
			<main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
				{isSubmitting && !isTeacher && !hasSubmitted && (
					<div className="mb-4 bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-md shadow-md">
						<div className="flex items-center gap-3">
							<div className="animate-spin rounded-full h-5 w-5 border-b-2 border-yellow-600"></div>
							<div>
								<p className="text-yellow-800 font-bold">Proses Pengumpulan Sedang Berlangsung</p>
						...
						>
							<div className="flex items-start gap-4">
								...
								<div className="flex items-center justify-between w-full">
									...
									{isGraded && (
										<div className="bg-blue-600 rounded-md border-2 border-yellow-500 px-5 py-2 inline-block">
